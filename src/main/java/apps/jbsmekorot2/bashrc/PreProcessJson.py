...
def expand_input(list_of_jbs_dict):
    ...
    for line in list_of_jbs_dict:
        ...
        splitted = re.split(', |\.', dic["text"])

        offset = 0
        index = 1
        splitted_file.append({"uri": dic["uri"], "offset": offset+line[1], "text": splitted[0]})

        for part in splitted[1:]:
            offset += len(splitted[index - 1])
            splitted_file.append({"uri": dic["uri"], "offset": offset+line[1], "text": splitted[index]})
            index += 1
