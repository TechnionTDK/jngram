 ...
 * After we apply the MergeNgramsGoUp manipulation, we may have
 ...
 */
public class RemoveTagsInContainedNgrams implements NgramDocumentManipulation {
    @Override
    public void manipulate(NgramDocument doc) {
        for (Ngram ng1 : doc.getAllNgrams()) {
            if (ng1.getTags().size() == 0)
                continue;
