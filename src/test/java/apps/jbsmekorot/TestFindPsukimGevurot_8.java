...
public class TestFindPsukimGevurot_8 {
    ...
    public void testTagsSpansLength2() {
        ...
        //assertTrue(doc.getNgram(122, 123).getSortedTags().contains("jbr:text-tanach-1-15-8"));
        //assertTrue(doc.getNgram(124, 125).getSortedTags().contains("jbr:text-tanach-1-15-8"));
        assertTrue(doc.getNgram(125, 126).getSortedTags().contains("jbr:text-tanach-1-15-8"));
        assertTrue(doc.getNgram(126, 127).getSortedTags().contains("jbr:text-tanach-1-15-8"));
        assertTrue(doc.getNgram(127, 128).getSortedTags().contains("jbr:text-tanach-1-15-8"));
    }
    ...
    public void testFinal() {
        ...
        // span2
        assertEquals(getList("jbr:text-tanach-1-15-8"), doc.getNgram(15, 16).getSortedTags());
        assertEquals(getList("jbr:text-tanach-1-15-8"), doc.getNgram(153, 154).getSortedTags());

        // span3
        // actually word "ki" does not belong to the context (but belongs to the pasuk...)
        assertEquals(getList("jbr:text-tanach-1-15-8"), doc.getNgram(8, 10).getSortedTags());

        // span4
        assertEquals(getList("jbr:text-tanach-1-15-8"), doc.getNgram(73, 76).getSortedTags());
        assertEquals(getList("jbr:text-tanach-1-15-8"), doc.getNgram(92, 95).getSortedTags());
        // here we do not catch the prefix of the pasuk, see daf for details
        assertEquals(getList("jbr:text-tanach-1-15-8"), doc.getNgram(125, 128).getSortedTags());
